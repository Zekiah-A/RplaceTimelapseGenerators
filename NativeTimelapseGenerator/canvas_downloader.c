...
struct canvas_metadata download_canvas_metadata(const char* metadata_url, CURL* curl_handle)
{
	...
	metadata.palette = malloc(3 * palette_size * sizeof(uint8_t));
...
DownloadedResult download_url(struct download_worker_data* worker_data)
{
	...
	char canvas_url[256];
	snprintf(canvas_url, sizeof(canvas_url), "%s/place", worker_data->current_canvas_info.base_url);
	...
	else
	{
		...
		snprintf(metadata_url, sizeof(metadata_url), "%s/metadata.json", worker_data->current_canvas_info.base_url);
