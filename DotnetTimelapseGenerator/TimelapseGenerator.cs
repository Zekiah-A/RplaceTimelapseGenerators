...
internal static class TimelapseGenerator
{
    ...
    public static async Task GenerateImageFrom(byte[] board, int width, int height, string directory, string name)
    {
        using var bitmap = new SKBitmap(width, height);

        for (var i = 0; i < board.Length; i++)
        {
            bitmap.SetPixel(i % width, i / width, Colours[board[i]]);
        }
